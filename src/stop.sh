...
case $MODE in  
  t|T) 
    ...
    echo "running command ${purple}docker-compose -p JRPC-$MODE_flag -f $JRPC/etc/$MODE_flag/Docker/docker-compose-$MODE_flag-minimal.yaml down ${reset}"
    
    docker-compose -p JRPC-$MODE_flag -f $JRPC/etc/$MODE_flag/Docker/docker-compose-$MODE_flag-minimal.yaml down ;;
